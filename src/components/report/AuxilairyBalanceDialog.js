...
function AuxilairyBalanceDialog(props) {
  const { open, onClose, type } = props;
  const [date, setDate] = React.useState([moment(), moment()]);
  const [devise, setDevise] = React.useState('cdf_cdf');
  // const [category, setCategory] = React.useState('global');
  ...
  const filtredCompte = React.useMemo(() => {
    if (comptes) {
      return getUniqueCompte(comptes, 'compte').filter(
        (d) =>
  ...
  return (
    ...
    >
      <DialogTitle style={{ cursor: 'move' }} id="draggable-dialog-title">
        Impression Balance Auxiliaire Des Comptes
      </DialogTitle>
      <Divider />
      <DialogContent>
      ...
      <DialogActions>
        ...
        <Button
          onClick={() => {
            ...
            const printData = {
              type,
              title: 'BALANCE AUXILIAIRE DES COMPTES',
              date: {
...
AuxilairyBalanceDialog.propTypes = {
  open: PropTypes.bool,
  onClose: PropTypes.func,
  type: PropTypes.string
};

export default AuxilairyBalanceDialog;
