...
const createRow = (data) => {
  const montant1 = _.ceil(data.montant1, 6);
  const montant2 = _.ceil(data.montant2, 6);

  return `
    <tr class="no-border">
      <td class="no-border-t no-border-b">${data.compt1}</td>
      <td class="no-border-t no-border-b no-border-r">${data.intitule1}</td>
      <td class="text-right no-border-t no-border-b no-border-l">${
        data.compt1 ? formatNumber(montant1) : ''
      }</td>
      <td class="no-border-t no-border-b">${data.compt2}</td>
      <td class="no-border-t no-border-b  no-border-r">${data.intitule2}</td>
      <td class="text-right no-border-t no-border-b no-border-l">${
        data.compt2 ? formatNumber(montant2) : ''
      }</td>
    </tr>
  `;
};
...
const createTable = (data) => {
  return `
    <thead>
      <tr class="no-border">
        <th style="background-color: white" rowspan="2">COMPTE</th>
        <th style="background-color: white" colspan="2" class="text-center no-border-b">ACTIF</th>
        <th style="background-color: white" rowspan="2">COMPTE</th>
        <th style="background-color: white" colspan="2" class="text-center no-border-b">PASSIF</th>
      </tr>
      <tr  class="no-border">
        <th style="background-color: white" class="no-border-t no-border-r">INTITULE</th>
        <th style="background-color: white" class="text-right no-border-t no-border-l">MONTANT</th>
        <th style="background-color: white" class="no-border-t no-border-r">INTITULE</th>
        <th style="background-color: white" class="text-right no-border-t no-border-l">MONTANT</th>
      </tr>
    </thead>
