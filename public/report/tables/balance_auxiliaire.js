...
const createRow = (data) => {
  ...
  const debit3 = debit2 > credit2 ? _.ceil(debit2 - credit2, 6) : 0;
  const credit3 = credit2 > debit2 ? _.ceil(credit2 - debit2, 6) : 0;
  const solde = _.ceil(debit1 + debit3 - (credit1 + credit3), 6);
...
const createRowFooter = (data) => `
  <tr>
    ...
    <th></th>
    <th>Total Général</th>
    <th class='text-right'>${formatNumber(data.debit1)}</th>
    <th class='text-right'>${formatNumber(data.credit1)}</th>
    <th class='text-right'>${formatNumber(data.debit)}</th>
    <th class='text-right'>${formatNumber(data.credit)}</th>
    <th class='text-right'>${formatNumber(data.debit)}</th>
    <th class='text-right'>${formatNumber(data.credit)}</th>
    <th class='text-right'>${formatNumber(
      _.ceil(data.debit1 + data.debit - (data.credit1 + data.credit))
    )}</th>
  </tr>  
`;
...
const createTable = (data) => ` 
   <thead>
    <tr>
      ...
      <th rowspan="2" style="background-color: white">Code</th>
      <th td="" rowspan="2" style="background-color: white">
        Intitulé
      </th>
      ...
      <th colspan="2" style="background-color: white" class="text-center">SOLDE DE LA PERIODE</th>
  ...
  <tbody>
  ${groupBalance(data, 3, 'compt2').map(createRow).join('')}
